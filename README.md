...
仓库目前为私有；普通 Raw 链接不能匿名下载。保持私有时，将两个 JS 文件下载并导入 Loon，配置中的两个 script-path 分别改为本地文件名。如果选择公开仓库，才使用 loon.conf 中的远程链接。
...
签到脚本仍访问吉利接口、Apple 版本查询接口，并可能从 cdnjs 下载 CryptoJS 4.1.1。备份避免依赖原作者仓库的后续可用性，但并非离线运行，接口变更或凭据过期仍会影响签到。
